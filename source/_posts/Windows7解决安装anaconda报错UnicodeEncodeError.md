---
layout: post
title: Windows7解决安装anaconda报错 install error：UnicodeEncodeError
categories: 数据分析
description: 在Windows上安装anaconda时，报install error：UnicodeEncodeError：'ascii' codec can't encode characters in position 3-6：ordinal not in range(128)。
date: 2017-9-16 19:12:24
tags: [机器学习,数据分析,anaconda]

---
# 问题描述
安装anaconda2的过程中报错：UnicodeEncodeError: 'ascii' codec can't encode characters in position 3-6: ordinal not in range(128)。
错误内容：
```
Traceback (most recent call last):
  File "..\Lib\_nsis.py ", line 159, in <module>
    main()
  File "..\Lib\_nsis.py ", line 133, in main
    mk_menus(remove=False)
  File "..\Lib\_nsis.py ", line 50, in mk_menus
    import menuinst
  File "D:\Anaconda2\lib\site-packages\menuinst\__init__.py", line 23, in <module>
    from .win32 import Menu, ShortCut
  File "D:\Anaconda2\lib\site-packages\menuinst\win32.py ", line 56, in <module>
    "documents": get_folder_path(FOLDERID.Documents),
  File "D:\Anaconda2\lib\site-packages\menuinst\knownfolders.py ", line 215, in get_folder_path
    return get_path(folder_id, user)
  File "D:\Anaconda2\lib\site-packages\menuinst\knownfolders.py ", line 206, in get_path
    path = path.decode(codec)
UnicodeEncodeError: 'ascii' codec can't encode characters in position 3-6: ordinal not in range(128)
```
错误截图：
<center>
![](http://otue1rxl3.bkt.clouddn.com/17-9-16/59834033.jpg)
</center>
# 解决方案
找到anaconda2的安装目录下的文件Lib\_nsis.py，用记事本打开
```python
...
```
到`import sys`之后即可解决问题。
*PS：可能在这个问题之后还会报failed to create anacoda menu 的错误，请参考[Windows7解决安装anaconda报错failed to create anacoda menu](https://lntuleej.github.io/2017/09/10/Windows7%E8%A7%A3%E5%86%B3%E5%AE%89%E8%A3%85anaconda%E6%8A%A5%E9%94%99failed%20to%20create%20anacoda%20menu/)*